...
export const useAuth = () => {
  ...
  const loginRef = useRef<
    (options?: MutationOptions<LoginCredentials>, retryAttempts?: number) => Promise<AuthApiResponse<LoginResponse> | null>
  >(() => Promise.resolve(null));
  const getAllUsersRef = useRef<
    (options?: MutationOptions<void>, retryAttempts?: number) => Promise<AuthApiResponse<UsersListResponse> | null>
  >(() => Promise.resolve(null));
  ...
  const login = useCallback(async (options: MutationOptions<LoginCredentials> = {}, retryAttempts = 0): Promise<AuthApiResponse<LoginResponse> | null> => {
    ...
    const url = customEndpoint || `${authBasePath}/login`;
    ...
    try {
      ...
      if (shouldRetry) {
        setTimeout(() => {
          if (isMountedRef.current) void loginRef.current(options, retryAttempts + 1);
  ...
  const register = useCallback(async (options: MutationOptions<RegisterUserData> = {}): Promise<AuthApiResponse<RegisterResponse> | null> => {
    ...
    const url = customEndpoint || `${authBasePath}/register`;
  ...
  const resetPassword = useCallback(async (options: MutationOptions<{ email: string }> = {}): Promise<void> => {
    ...
    const url = customEndpoint || `${authBasePath}/reset-password`;
  ...
  const changePassword = useCallback(async (options: MutationOptions<ChangePasswordData> = {}): Promise<AuthApiResponse<ChangePasswordResponse> | null> => {
    ...
    const url = customEndpoint || `${authBasePath}/change-password`;
  ...
  const getAllUsers = useCallback(async (options: MutationOptions<void> = {}, retryAttempts = 0): Promise<AuthApiResponse<UsersListResponse> | null> => {
    ...
    const url = customEndpoint || `${authBasePath}/users`;
    ...
    try {
      ...
      if (shouldRetry) {
        setTimeout(() => {
          if (isMountedRef.current) void getAllUsersRef.current(options, retryAttempts + 1);
        }, retryDelay * Math.pow(2, retryAttempts));
        return null;
      }
